...
RSpec.describe SpaceRepository do
  ...
  describe '#space_from_record' do
    it 'returns a Space object from a SQL record' do
      ...
      expect(space.start_date).to eq Date.new(2022,11,01)
      expect(space.end_date).to eq Date.new(2022,12,01)
    ...
    it 'add a space to the database' do
      ...
      expect(repo.all).to include(
        have_attributes(
          ...
          price_night: 150.0,
          user_id: 4,
          start_date: Date.new(2023,01,01),
          end_date: Date.new(2023,01,28)
        )
      )
    end
  end
